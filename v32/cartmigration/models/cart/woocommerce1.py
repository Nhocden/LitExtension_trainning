...
class LeCartWoocommerce(LeCartWordpress):
	...
	def display_config_source(self):
		...
		self._notice['src']['support']['custom_order_statuses'] = False
		self._notice['src']['support']['yith_woocommerce_custom_order_status_premium'] = False
		...
		if active_plugins and active_plugins['data']:
			...
			if active_plugin_v:
				...
				if active_plugin_v_data:
					...
					if active_plugin_v_data and 'custom-order-statuses-woocommerce/custom-order-statuses-for-woocommerce.php' in active_plugin_v_data:
						self._notice['src']['support']['custom_order_statuses'] = True
					if active_plugin_v_data and 'yith-woocommerce-custom-order-status-premium/init.php' in active_plugin_v_data:
						self._notice['src']['support']['yith_woocommerce_custom_order_status_premium'] = True
		...
		if self._notice['src']['support']['yith_woocommerce_custom_order_status_premium']:
			queries_config['orders_status']['query'] = "SELECT * FROM `_DBPRF_posts` WHERE `post_type` = 'yith-wccos-ostatus' AND `post_status` = 'publish'"

		if self._notice['src']['support']['custom_order_statuses']:
		...
		if config and config['result'] == 'success':
			if config['data']['orders_status']:
				for order_status_row in config['data']['orders_status']:
					...
					if self._notice['src']['support']['custom_order_status']:
						...
						order_status_data[order_status_id] = order_status_row['post_title']
					elif self._notice['src']['support']['yith_woocommerce_custom_order_status_premium']:
			...
			elif self._notice['src']['support']['custom_order_statuses'] and config['data']['custom_order_statuses'] and to_len(config['data']['custom_order_statuses']):
	...
	def display_config_target(self):
		...
		if active_plugins and active_plugins['data']:
			...
			if active_plugin_v:
				...
				if ('polylang-pro/polylang.php' in active_plugin_v_data) or ('polylang/polylang.php' in active_plugin_v_data):
		...
		if config and config['result'] == 'success':
			...
			if self._notice['target']['support']['user_role_editor'] and config['data'].get('user_role_editor'):
				...
				user_role_data = dict()
				none_select = ('administrator', 'editor', 'author', 'contributor', 'shop_manager', 'wpseo_editor', 'wpseo_manager', 'translator')
			...
			if self._notice['target']['support'].get('plugin_order_status') and config['data']['custom_order_status'] and to_len(config['data']['custom_order_status']) > 0:
			...
			elif self._notice['target']['support']['custom_order_statuses'] and config['data']['custom_order_statuses'] and to_len(config['data']['custom_order_statuses']):
				data = php_unserialize(config['data']['custom_order_statuses'][0]['option_value'])
				order_status_data = data
	...
	def category_import(self, convert, category, categories_ext):
		...
		parent_id = 0
		# slug_name = convert['name'].replace("- ", "")
		slug_name = convert['url_key'] if convert.get('url_key') else (convert['code'] if convert['code'] and convert['id'] else (convert['name'].lower() if convert['name'] else ''))
		# slug = self.sanitize_title(convert['name'])
		slug = self.sanitize_title(slug_name.replace("  ", " "))
		if slug:
			while self.check_slug_exist(slug):
		...
		if convert['parent'] and (convert['parent']['id'] or convert['parent']['code']):
			...
			if parent_import['result'] == 'success':
				parent_id = self.get_map_field_by_src(self.TYPE_CATEGORY if not self.blog_running else self.TYPE_CATEGORY_BLOG, convert['parent']['id'], to_str(convert['parent']['code']), language_code, 'value')
		# parent_import['data']
		category_term = {
		...
		if convert['thumb_image']['url'] or convert['thumb_image']['path']:
			image_process = self.process_image_before_import(convert['thumb_image']['url'], convert['thumb_image']['path'])
			image_import_path = self.uploadImageConnector(image_process, self.add_prefix_path(self.make_woocommerce_image_path(image_process['path'], 'category'), self._notice['target']['config']['image_category'].rstrip('/')))
			if image_import_path:
				cate_image = self.remove_prefix_path(image_import_path, self._notice['target']['config']['image_category'])
				image_details = self.get_sizes(image_process['url'])
			...
			if (self._notice['config']['seo'] or self._notice['config']['seo_301']) and convert['seo']:
				...
				for seo_url in convert['seo']:
					...
					if seo_url['request_path']:
						...
						if check_seo_exit and check_seo_exit['result'] == 'success' and check_seo_exit['data'] and to_len(check_seo_exit['data']) > 0:
							continue
						else:
							le_url_rewrite = {
							...
							self.import_data_connector(self.create_insert_query_connector("lecm_rewrite", le_url_rewrite), 'seo_category')
	...
	def after_category_import(self, category_id, convert, category, categories_ext):
		...
		if self.is_polylang():
			...
			if taxonomy_lang_id:
				...
				query_lang = {
					'type': 'select',
					'query': "SELECT * FROM `_DBPRF_term_taxonomy` WHERE `taxonomy` = 'term_translations' AND term_taxonomy_id = " + to_str(taxonomy_lang_id)
				}
				cate_lang_data = self.get_connector_data(self.get_connector_url('query'), {'query': json.dumps(query_lang)})
				if cate_lang_data and cate_lang_data['data']:
					...
					term_relationships_lang = self.create_insert_query_connector('term_relationships', term_relationships)
					all_queries.append(term_relationships_lang)

					relation_lang_code = {
						'object_id': term_id,
						'term_taxonomy_id': self._notice['target']['support']['polylang_category'].get(language_code, 0),
						'term_order': 0,
					}
					relation_lang_code_query = self.create_insert_query_connector('term_relationships', relation_lang_code)
			...
			if not convert.get('language_code'):
				list_target_id = list()
				for src_language_id, target_language_id in self._notice['map']['languages'].items():
					if target_language_id in list_target_id or to_str(target_language_id) == to_str(self._notice['target']['language_default']) or not convert['languages'].get(src_language_id):
						continue
					list_target_id.append(target_language_id)
					category_lang = self.get_convert_data_language(convert, src_language_id)
					category_lang['language_code'] = target_language_id
	...
	def get_products_ext_export(self, products):
		...
		product_ext_rel_third_queries = {
			'image': {
				'type': 'select',
				'query': "SELECT p.ID, p.post_title, pm.meta_value, p.guid, p.post_date FROM _DBPRF_posts AS p "
	...
	def convert_product_export(self, product, products_ext):
		...
		if thumbnail_id:
			...
			if thumbnail_src:
				...
				product_data['thumb_image']['created_at'] = to_str(thumbnail_src[0]['post_date'])
		...
		if gallery_ids:
			...
			for gallery_id in gallery_ids:
				...
				if image_gallery_src:
					...
					product_image_data['created_at'] = image_gallery_src[0]['post_date']
	...
	def product_import(self, convert, product, products_ext):
		...
		if self.is_wpml() and not language_code or (self.is_polylang() and not language_code):
			language_code = self._notice['target']['language_default']
		code_product = convert['url_key'] if convert['url_key'] and convert['id'] else (convert['name'].lower() if convert['name'] else '')
		...
		while check_slug_exist:
			check_slug_exist = True if self.select_map(self._migration_id, self.TYPE_PRODUCT, None, None, None, code_product) else False
			if check_slug_exist:
		...
		product_query = self.create_insert_query_connector('wp_posts', product_data)
		...
		# 		check_sku_exist = True if self.select_map(self._migration_id, self.TYPE_PRODUCT, None, None, None, None, sku_product) else False
		# 		if check_sku_exist:
		# 			sku_product += '-' + to_str(to_int(time.time()))
		# convert['sku'] = sku_product
		#
		# self.insert_map(self.TYPE_PRODUCT, convert['id'], product_import, convert['code'] if convert['code'] else code_product, code_product, sku_product, language_code)
	...
	# 			image_details = self.get_sizes(image_process['url'])
	# 			thumbnail_id = self.wp_image(product_image, image_details, convert['thumb_image'].get('label', ''), time = convert['thumb_image']['created_at'])
	...
	# 				product_image = self.remove_prefix_path(image_import_path, self._notice['target']['config']['image_product'])
	# 				image_details = self.get_sizes(image_process['url'])
	# 				img_id = self.wp_image(product_image, image_details, image['label'], time = image['created_at'])
	# 				if img_id:
	...
	# 			relate_desc_id = self.get_map_field_by_src(self.TYPE_PRODUCT, relate_src['id'], relate_src['code'], language_code)
	# 			if not relate_desc_id:
	# 				relate_desc_id = self.get_map_field_by_src(self.TYPE_PRODUCT, None, relate_src['code'], language_code)
	# 			if relate_desc_id:
	...
	# 				'file': self._notice['target']['cart_url'] + "/wp-content/uploads/downloadable/" + to_str(download_data.get('path')).strip('/'),
	# 			}
	# 			downloadable_files[id_download] = download_item
	#
	# 	# download_name = download_data['name']
	# 	# n = download_name.rfind('.')
	# 	# file_type = download_name[n+1:]
	# 	# post_download = {
	# 	# 	'post_author': 1,
	# 	# 	'post_date': convert['created_at'] if convert['created_at'] else get_current_time(),
	# 	# 	'post_date_gmt': convert['created_at'] if convert['created_at'] else get_current_time(),
	# 	# 	'post_content': '',
	# 	# 	'post_title': self.strip_html_tag(download_data['name']),
	# 	# 	'post_excerpt': '',
	# 	# 	'post_status': "inherit",
	# 	# 	'comment_status': 'open',
	# 	# 	'ping_status': 'closed',
	# 	# 	'post_password': '',
	# 	# 	'post_name': self.strip_html_tag(download_data['name']),
	# 	# 	'to_ping': '',
	# 	# 	'pinged': '',
	# 	# 	'post_modified': convert['updated_at'] if convert['updated_at'] else get_current_time(),
	# 	# 	'post_modified_gmt': convert['updated_at'] if convert['updated_at'] else get_current_time(),
	...
	# 	# 	'guid': self._notice['target']['cart_url'] + "/wp-content/uploads/woocommerce_uploads/" + download_data['file'],
	# 	# 	'menu_order': convert.get('menu_order', 0),
	# 	# 	'post_type': "attachment",
	# 	# 	'post_mime_type': 'application/' + file_type,
	# 	# 	'comment_count': 0,
	# 	# 	# 'sort_order': get_value_by_key_in_dict(convert, 'sort_order', 0),
	# 	# }
	# 	# product_query = self.create_insert_query_connector('posts', post_download)
	# 	# download_id = self.import_product_data_connector(product_query, True, convert['id'])
	# 	# if download_id:
	# 	# 	postmeta_download = {
	# 	# 		'post_id': download_id,
	# 	# 		'meta_key': '_wp_attached_file',
	# 	# 		'meta_value': "woocommerce_uploads/" + download_data['file'],
	# 	# 	}
	# 	# 	product_query = self.create_insert_query_connector('postmeta', postmeta_download)
	# 	# 	self.import_product_data_connector(product_query, True, convert['id'])
	#
	# 	# self.log(downloadable_files, 'downloadable_files')
	# 	if convert['relate']['parent']:
	# 		for upsell_src in convert['relate']['parent']:
	# 			parent_desc_id = self.get_map_field_by_src(self.TYPE_PRODUCT, upsell_src['id'], upsell_src['code'], language_code)
	# 			if not parent_desc_id:
	# 				parent_desc_id = self.get_map_field_by_src(self.TYPE_PRODUCT, None, upsell_src['code'], language_code)
	# 			if parent_desc_id:
	...
	# 					}
	# 					parent_meta_upsell = self.get_connector_data(self.get_connector_url('query'), {'query': json.dumps(query)})
	...
	# 					}
	# 					parent_meta_crosssell = self.get_connector_data(self.get_connector_url('query'), {'query': json.dumps(query)})
	...
	# 						else:
	# 							new_value.append(product_id)
	# 						new_query = "UPDATE _DBPRF_postmeta set meta_value = '" + php_serialize(new_value) + "' WHERE post_id = " + to_str(parent_desc_id) + " AND meta_key = '_crosssell_ids'"
	# 						all_queries.append({
	# 							'type': 'query',
	# 							'query': new_query
	# 						})
	...
	# 		'_virtual': "yes" if convert['type'] == self.PRODUCT_VIRTUAL else "no",
	# 		'_regular_price': convert['price'],
	# 		'_sale_price': convert['special_price']['price'] if convert['special_price']['price'] and (to_int(self.to_timestamp(convert['special_price']['end_date'])) > to_int(time.time()) or convert['special_price']['end_date'] == '' or convert['special_price']['end_date'] == '0000-00-00' or convert['special_price']['end_date'] == '0000-00-00 00:00:00' or convert['special_price']['end_date'] == None) else "",
	# 		'_tax_status': convert['tax'].get('status', ("taxable" if to_int(convert['tax']['id']) or convert['tax']['code'] else "none")),
	# 		'_tax_class': tax_class if tax_class else '',
	# 		'_featured': "no",
	...
	# 		product_meta['_sale_price_dates_to'] = self.to_timestamp(convert['special_price']['end_date'])
	# 	# if self._notice['target']['support']['yoast_seo']:
	# 	product_meta['_yoast_wpseo_title'] = to_str(convert['meta_title']).replace('%', '') if 'meta_title' in convert and convert['meta_title'] else ''
	# 	product_meta['_yoast_wpseo_metadesc'] = to_str(convert['meta_description']).replace('%', '') if 'meta_description' in convert and convert['meta_description'] else ''
	#
	...
	# 				child_bundle_id = self.get_map_field_by_src(self.TYPE_PRODUCT, bundle_selection['product_id'], None, self._notice['target']['language_default'])
	# 				if not child_bundle_id:
	# 					continue
	# 				if product_meta['woosb_ids'] == '':
	# 					product_meta['woosb_ids'] = to_str(child_bundle_id) + '/' + to_str(to_int(to_decimal(bundle_selection['selection_qty'])))
	# 				else:
	# 					product_meta['woosb_ids'] = to_str(product_meta['woosb_ids']) + ',' + to_str(child_bundle_id) + '/' + to_str(to_int(to_decimal(bundle_selection['selection_qty'])))
	...
	# 				'product_tag': {
	# 					'type': 'select',
	# 					'query': "SELECT * FROM _DBPRF_terms AS t JOIN _DBPRF_term_taxonomy As tt ON t.term_id = tt.term_id WHERE tt.taxonomy = 'product_tag' AND t.name IN " + self.list_to_in_condition(tags_src)
	# 				}
	# 			}
	...
	# 						'count': 0
	# 					}
	# 					tag_id = self.import_data_connector(self.create_insert_query_connector('term_taxonomy', value_term_taxonomy), 'products')
	...
	# 					query = {
	# 						'type': 'select',
	# 						'query': "SELECT * FROM _DBPRF_postmeta WHERE post_id = " + to_str(parent_id_import) + "  AND meta_key = '_children'"
	# 					}
	...
	# 						new_value.append(product_id)
	# 						new_query = "UPDATE _DBPRF_postmeta set meta_value = '" + php_serialize(new_value) + "' WHERE post_id = " + to_str(parent_id_import) + " AND meta_key = '_children'"
							all_queries.append({
								'type': 'query',
								'query': new_query
							})
							...
							new_value = list()
							new_value.append(product_id)
							new_query = "INSERT INTO _DBPRF_postmeta (post_id, meta_key, meta_value) VALUES ( " + to_str(parent_id_import) + ", '_children', '" + php_serialize(new_value) + "')"
							all_queries.append({
								'type': 'query',
								'query': new_query
							})
			...
			if convert['children']:
				...
				if convert['options']:
					...
					if self.count_child_from_option(convert['options']) <= self.VARIANT_LIMIT:
						children_list = self.convert_option_to_child(option_list, convert)
			if children_list and option_list and not self._notice['target']['support'].get('addons') and to_len(children_list) <= self.VARIANT_LIMIT:
			...
			if self._notice['target']['support']['addons'] and not self.is_woo2woo():
				...
				for option in option_list:
					...
					if self.is_wpml() and language_code:
						option_data = self.get_convert_data_language(option_data, None, language_code, 'option_languages')
					...
					for value in option['values']:
						option_value = value
						if self.is_wpml() and language_code:
							option_value = self.get_convert_data_language(option_value, None, language_code, 'option_value_languages')
						value_options_addons = dict()
						value_options_addons['label'] = option_value['option_value_name']
						value_options_addons['price'] = value['option_value_price'] if to_int(value['option_value_price']) > 0 else ''
						...
						if option_value.get('option_value_image'):
							...
							image_option_path = self.uploadImageConnector(option_image_process, self.add_prefix_path(self.make_woocommerce_image_path(option_image_process['path']), self._notice['target']['config']['image'].rstrip('/') + '/options'))
				...
				if ((children_list and to_len(children_list) > self.VARIANT_LIMIT) or (not children_list and self.count_child_from_option(convert['options']) > self.VARIANT_LIMIT)) and ((not self.is_wpml() and not self.is_polylang()) or not convert.get('language_code')):
				...
				if children_list and to_len(children_list) <= self.VARIANT_LIMIT:
						...
						for option in option_list:
							...
							if option['values']:
								for attr_value in option['values']:
									opt_value_id = self.get_woo_attribute_value(attr_value['option_value_name'], pro_attr_code, language_code, attr_value)
									if not opt_value_id:
										continue
									where_count_update = {
										'term_taxonomy_id': opt_value_id,
					...
					for key_child, product_child in enumerate(children_list):
						children_data = {
							'post_author': 1,
							'post_date': convert_format_time(product_child['created_at']) if product_child['created_at'] and '0000-00-00' not in to_str(product_child['created_at']) else get_current_time(),
							'post_date_gmt': convert_format_time(product_child['created_at']) if product_child['created_at'] and '0000-00-00' not in to_str(product_child['created_at']) else get_current_time(),
							'post_content': "",
							...
							'to_ping': '',
							'pinged': '',
							'post_modified': product_child['updated_at'] if product_child['updated_at'] else get_current_time(),
							'post_modified_gmt': product_child['updated_at'] if product_child['updated_at'] else get_current_time(),
							'post_content_filtered': '',
							'post_parent': product_id,
							'guid': self._notice['target']['cart_url'].rstrip('/') + "/?product_variation=product-" + to_str(
						...
						if not children_id:
							msg = self.warning_import_entity('Product_variable', product_child['id'], product_child['code'])
						...
						if product_child['thumb_image']['url'] or product_child['thumb_image']['path']:
							image_process = self.process_image_before_import(product_child['thumb_image']['url'], product_child['thumb_image']['path'])
							image_import_path = self.uploadImageConnector(image_process, self.add_prefix_path(self.make_woocommerce_image_path(image_process['path']), self._notice['target']['config']['image_product'].rstrip('/')))
							if image_import_path:
								product_image = self.remove_prefix_path(image_import_path, self._notice['target']['config']['image_product'])
								image_details = self.get_sizes(image_process['url'])
						...
						if product_child.get('is_in_stock'):
							child_stock_status = 'instock' if product_child['is_in_stock'] and to_int(product_child['qty']) > 0 else 'outofstock'
						else:
							child_stock_status = 'outofstock' if product_child['manage_stock'] and to_int(product_child['qty']) <= 0 else 'instock'
						if self.is_woo2woo():
						...
						children_meta = {
							...
							'_stock': product_child['qty'] if product_child['qty'] else 0,
							'_regular_price': product_child['price'],
							'_sale_price': product_child['special_price']['price'] if product_child['special_price']['price'] and (self.to_timestamp(product_child['special_price']['end_date']) > time.time() or (product_child['special_price']['end_date'] == '0000-00-00' or product_child['special_price']['end_date'] == '0000-00-00 00:00:00' or product_child['special_price']['end_date'] == '' or product_child['special_price']['end_date'] == None or convert['special_price']['end_date'] == None)) else '',
							'_price': product_child['special_price']['price'] if product_child['special_price']['price'] and (self.to_timestamp(product_child['special_price']['end_date']) > time.time() or (product_child['special_price']['end_date'] == '0000-00-00' or product_child['special_price']['end_date'] == '0000-00-00 00:00:00' or product_child['special_price']['end_date'] == '' or product_child['special_price']['end_date'] == None or convert['special_price']['end_date'] == None)) else product_child['price'],
							'_download_limit': "",
							'_download_expiry': "",
							'_downloadable_files': "",
							'_variation_description': product_child.get('description', '') if product_child.get('description', '') != convert['description'] else ''
						}
						if convert['tax']['id'] or convert['tax']['code']:
							tax_class = self.get_map_field_by_src(self.TYPE_TAX, convert['tax']['id'], convert['tax']['code'], field = 'code_desc')
						...
						if product_child['special_price']['price'] and (self.to_timestamp(product_child['special_price']['end_date']) > time.time() or (product_child['special_price']['end_date'] == '0000-00-00' or product_child['special_price']['end_date'] == '0000-00-00 00:00:00' or convert['special_price']['end_date'] == None)) or product_child['special_price']['end_date'] == '':
							if product_child['special_price']['start_date']:
								children_meta['_sale_price_dates_from'] = self.to_timestamp(product_child['special_price']['start_date'])
							if product_child['special_price']['end_date']:
								children_meta['_sale_price_dates_to'] = self.to_timestamp(product_child['special_price']['end_date'])
						...
						if self._notice['target']['support'].get('wpml_currency'):
							...
							for language_child, data_language_child in product_child['languages'].items():
								...
								if not language_child_target or to_str(language_child_target) == to_str(language_code):
									continue
								lang_child_currency_switch = self._notice['target']['currency_map'].get(to_str(language_child_target))
								if not lang_child_currency_switch:
									continue
								children_meta['_regular_price_' + to_str(lang_child_currency_switch)] = data_language_child['price']
								...
								children_meta['_price_' + to_str(lang_child_currency_switch)] = data_language_child['price']
						...
						children_meta_lookup_query = self.create_insert_query_connector('wc_product_meta_lookup', children_meta_lookup)
						...
						if self.is_wpml():
							...
							if not convert.get('language_code'):
								...
								self.insert_map('trid', convert['id'], product_variant_trid, convert['code'], None, key_check)
							else:

								product_variant_trid_map = self.select_map(self._migration_id, 'trid', convert['id'], None, convert['code'], None, key_check)
							...
							if product_variant_trid:
								...
								wpml_child_query = self.create_insert_query_connector("icl_translations", wpml_child_data)
		...
		if self.is_wpml():
			...
			if not trid:
				trid = self.get_new_trid()
			default_lang_map = get_value_by_key_in_dict(self._notice['map']['languages'], get_value_by_key_in_dict(convert, 'language_default', self._notice['src']['language_default']), self._notice['target']['language_default'])
			wpml_default_data = {
		...
		if self.is_polylang():
			...
			if post_trans_id:
				...
				query_lang = {
					'type': 'select',
					'query': "SELECT * FROM `_DBPRF_term_taxonomy` WHERE `taxonomy` = 'post_translations' AND term_taxonomy_id = " + to_str(post_trans_id)
				}
				product_lang_data = self.get_connector_data(self.get_connector_url('query'), {'query': json.dumps(query_lang)})
	...
	def customer_import(self, convert, customer, customers_ext):
		...
		customer_data = {
			...
			'user_registered': convert_format_time(convert['created_at']) if convert['created_at'] else get_current_time(),
			'user_status': 1 if convert.get('active') else 0,
			'display_name': get_value_by_key_in_dict(convert, 'display_name', to_str(convert['first_name']) + ' ' + to_str(convert['last_name']))
		}
	...
	def after_customer_import(self, customer_id, convert, customer, customers_ext):
		...
		user_meta_import_first = {
			...
			'_DBPRF_user_level': 0,
			'wc_points_balance': convert['point_rewards']['points_balance'] if convert.get('point_rewards') and convert['point_rewards'].get('points_balance') else 0,
		...
		if convert['address']:
			...
			user_meta = {
				...
				'billing_state': bill_data['state']['state_code'] if bill_data['state']['state_code'] else bill_data['state']['name'],
				'billing_postcode': bill_data['postcode'],
				'billing_email': convert['email'],
				'billing_phone': get_value_by_key_in_dict(bill_data, 'telephone', get_value_by_key_in_dict(convert, 'telephone', '')),
				...
				'shipping_state': ship_data['state']['state_code'] if ship_data['state']['state_code'] else ship_data['state']['name'],
		...
		if self._notice['target']['support'].get('plugin_woo_admin') or self.convert_version(self._notice['target']['config']['version'], 2) > 399:
			wc_customer_lookup_data = {
				...
				'state': convert['address'][0]['state']['state_code'] if to_len(convert['address']) > 0 and convert['address'][0]['state']['state_code'] else '',
			}
			self.import_data_connector(self.create_insert_query_connector('wc_customer_lookup', wc_customer_lookup_data), 'customer')
			...
			if convert.get('point_rewards'):
				...
				if convert['point_rewards'].get('reward_point'):
					for reward_point in convert['point_rewards']['reward_point']:
						...
						all_query_second.append(self.create_insert_query_connector("wc_points_rewards_user_points", wc_points_rewards_user_points))
				if convert['point_rewards'].get('reward_point_log'):
					for reward_point_log in convert['point_rewards']['reward_point_log']:
						...
						if reward_point_log.get('user_points_id'):
							user_points_id = self.get_map_field_by_src(self.TYPE_CUSTOMER, reward_point_log['user_points_id'])
						...
						all_query_second.append(self.create_insert_query_connector("wc_points_rewards_user_points_log", wc_points_rewards_user_points_log))
				if all_query_second:
	...
	def convert_order_export(self, order, orders_ext):
		...
		for order_product in order_products:
			...
			if to_int(self.get_value_metadata(order_product_metas, '_variation_id', self.get_value_metadata(order_product_metas, '_product_id', 0))) == 0:
	...
	def order_import(self, convert, order, orders_ext):
		...
		order_status = self._notice['map']['order_status'][convert['status']] if convert['status'] in self._notice['map']['order_status'] else 'wc-completed'
	...
	def after_order_import(self, order_id, convert, order, orders_ext):
		...
		order_meta = {
			...
			'_billing_email': get_value_by_key_in_dict(billing_address, 'email', get_value_by_key_in_dict(convert['customer'], 'email', '')),
			'_billing_phone': get_value_by_key_in_dict(billing_address, 'telephone', ''),
			'_shipping_country': get_value_by_key_in_dict(shipping_address['country'], 'country_code', '') if shipping_address and shipping_address['country'] else '',
			...
			'_shipping_state': get_value_by_key_in_dict(shipping_address['state'], 'state_code', shipping_address['state']['name']) if shipping_address and shipping_address['state'] else '',
			'_shipping_postcode': get_value_by_key_in_dict(shipping_address, 'postcode', ''),
			'_shipping_email': get_value_by_key_in_dict(shipping_address, 'email', get_value_by_key_in_dict(convert['customer'], 'email', '')),
		...
		if billing_address:
			...
			for key in ['_billing_first_name', '_billing_last_name', '_billing_country', '_billing_company', '_billing_address_1', '_billing_address_2', '_billing_city', '_billing_state', '_billing_postcode', '_billing_phone', '_billing_email']:
				if order_meta.get(key):
					order_meta['_billing_address_index'] += to_str(order_meta[key]) + ' '

		if shipping_address:
			order_meta['_shipping_address_index'] = ''
			for key in ['_shipping_first_name', '_shipping_last_name', '_shipping_country', '_shipping_company', '_shipping_address_1', '_shipping_address_2', '_shipping_city', '_shipping_state', '_shipping_postcode', '_shipping_phone', '_shipping_email']:
				if order_meta.get(key):
				...
				if product_download_data:
					for key, download_data in product_download_data.items():
						...
						for item_download in item.get('downloads', []):
							name = self._notice['target']['cart_url'] + "/wp-content/uploads/downloadable/" + item_download.get('file')
							if file == name:
						...
						order_query = self.create_insert_query_connector("woocommerce_downloadable_product_permissions", woocommerce_downloadable_product_permissions)
		...
		if shipping['amount'] != '' and shipping['amount'] != 0:
			order_shipping_data = {
		...
		for note in notes:
			if note.get('comment'):
				order_note_data = {
					...
					'comment_author': "admin" if 'customer_note' in note and not note['customer_note'] else 'WooCommerce',
		...
		if convert.get('payment'):
			...
			if payment_method.get('title'):
				order_note_data_payment_method = {
					...
					'comment_date': get_current_time(),
					'comment_date_gmt': get_current_time(),
					'comment_content': 'Payment Method: ' + to_str(get_value_by_key_in_dict(payment_method, 'title', '')),
					'comment_karma': 0,
					'comment_approved': 1,
					'comment_agent': "WooCommerce",
					'comment_type': "order_note",
				...
				order_note_data_payment_method_query = self.create_insert_query_connector("comments", order_note_data_payment_method)
		...
		if convert.get('shipping'):
			...
			if shipping_method.get('title'):
				order_note_data_shipping_method = {
					...
					'comment_date': get_current_time(),
					'comment_date_gmt': get_current_time(),
					'comment_content': 'Shipping Method: ' + to_str(get_value_by_key_in_dict(shipping_method, 'title', '')),
					'comment_karma': 0,
					'comment_approved': 1,
					'comment_agent': "WooCommerce",
					'comment_type': "order_note",
				...
				order_note_data_shipping_method_query = self.create_insert_query_connector("comments", order_note_data_shipping_method)
		...
		if convert.get('shipping_payment'):
			...
			if shiping_payment.get('title'):
				order_note_data_shiping_payment = {
					...
					'comment_date': get_current_time(),
					'comment_date_gmt': get_current_time(),
					'comment_content': 'Shipping Payment: ' + to_str(get_value_by_key_in_dict(shiping_payment, 'title', '')),
					'comment_karma': 0,
					'comment_approved': 1,
					'comment_agent': "WooCommerce",
					'comment_type': "order_note",
				...
				order_note_data_shiping_payment_query = self.create_insert_query_connector("comments", order_note_data_shiping_payment)
			...
			if convert.get('point_rewards'):
				if convert['point_rewards'].get('reward_point'):
					for reward_point in convert['point_rewards']['reward_point']:
						...
						all_queries.append(self.create_insert_query_connector("wc_points_rewards_user_points", wc_points_rewards_user_points))
				if convert['point_rewards'].get('reward_point_log'):
					for reward_point_log in convert['point_rewards']['reward_point_log']:
						user_id = 0
						if reward_point_log.get('user_points_id'):
						...
						all_queries.append(self.create_insert_query_connector("wc_points_rewards_user_points_log", wc_points_rewards_user_points_log))
		...
		if self._notice['target']['support'].get('plugin_woo_admin') or self.convert_version(self._notice['target']['config']['version'], 2) > 399:
			wc_order_stats_data = {
				...
				'status': self._notice['map']['order_status'][convert['status']] if convert['status'] in self._notice['map']['order_status'] else 'wc-completed',
		...
		for attribute in combination:
			if not check_any:
				if to_str(child.get(attribute['option_name'])) != to_str(attribute['option_value_name']):
					if to_str(child.get(to_str(attribute['option_code']).replace(' ', '-'))) != to_str(attribute['option_value_name']) and to_str(child.get(to_str(attribute['option_code']).replace(' ', '-').replace('/', '-').replace('---', '-'))) != to_str(attribute['option_value_name']):
						return False
			elif to_str(child.get(attribute['option_name'])) or to_str(child.get(to_str(attribute['option_code']).replace(' ', '-'))):
