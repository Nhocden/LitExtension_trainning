...
class LeCartCustom(LeBasecart):
	...
	def get_categories_ext_export(self, categories):
		...
		query = {
			'type': 'select',
			'query': "SELECT * FROM  categories_description WHERE categories_id > " + to_str(
				id_src) + " ORDER BY categories_id ASC LIMIT " + to_str(limit)
		}
		category_description = self.select_data_connector(query)
		language_ids = duplicate_field_value_from_list(category_description['data'], 'language_id')
		categories_ext_queries = {
			...
			'languages': {
				'type': 'select',
				'query': "SELECT * FROM languages WHERE languages_id IN " + self.list_to_in_condition(language_ids)
			},
			# 'cate_imgs': {
			# 	'type': 'select',
	...
	def convert_category_export(self, category, categories_ext):
		...
		category_data['updated_at'] = category['last_modified'] if category['last_modified'] else '0000-00-00 00:00:00'
		...
		languages = categories_ext['data']['languages']
		category_data['languages'] = languages
	...
	def get_products_ext_export(self, products):
		...
		product_ext_queries = {
			'products_description': {
				'type': "select",
				'query': "SELECT * FROM  products_description WHERE products_id IN " + self.list_to_in_condition(product_ids)
			},
			'category_product': {
				'type': "select",
				'query': "SELECT * FROM categories INNER JOIN products_to_categories on categories.categories_id = products_to_categories.categories_id WHERE products_to_categories.products_id IN  " + self.list_to_in_condition(product_ids)
			},
			'languages_product': {
				'type': "select",
				'query': "SELECT * FROM languages INNER JOIN products_description on languages.	languages_id = products_description.language_id WHERE products_description.products_id IN  " + self.list_to_in_condition(product_ids)
			}
	...
	def convert_product_export(self, product, products_ext):
		products_description = get_row_from_list_by_field(products_ext['data']['products_description'],'products_id', product['products_id'])
		product_data = self.construct_product()
		product_categories = get_list_from_list_by_field(products_ext['data']['category_product'], 'products_id',
			product['products_id'])
		...
		languages_product = get_list_from_list_by_field(products_ext['data']['languages_product'], 'products_id',
			product['products_id'])
		for languages_product in languages_product:
			product_language_data = self.construct_product_lang()
			product_language_data['name'] = languages_product['name']
			language_id = languages_product['languages_id']
		...
		product_data['image'] = product['products_image']
		product_data['name'] = products_description['products_name']
		product_data['description'] = products_description['products_description']
		...
		product_data['updated_at'] = product['products_last_modified'] if product['products_last_modified'] else '0000-00-00 00:00:00'
	...
	def prepare_customers_import(self):
		# query = {
		# 	'type': 'query',
		# 	'query': "ALTER TABLE customers MODIFY COLUMN password varchar(255)"
		# }
	...
	def get_customers_ext_export(self, customers):
		...
		customer_ext_queries = {
			...
			'address_book': {
				'type': 'select',
				'query': "SELECT * FROM  address_book INNER JOIN customers ON address_book.customers_id =customers.customers_id WHERE customers.customers_id IN " + self.list_to_in_condition(customers_ids)
			}
		}
	...
	def get_orders_ext_export(self, orders):
		...
		customers_id = duplicate_field_value_from_list(orders['data'], 'client_customers_id')
		currency = duplicate_field_value_from_list(orders['data'], 'currency')
		orders_ext_queries = {
			'order_items': {
				'type': 'select',
			...
			'orders_customer': {
				'type': 'select',
				'query': "SELECT * FROM  customers WHERE customers_id  IN " + self.list_to_in_condition(customers_id)
			},
			'orders_payment': {
				'type': 'select',
				'query': "SELECT * FROM  currencies WHERE 	code  IN " + self.list_to_in_condition(currency)
			},
			'countries': {
				'type': 'select',
				'query': "SELECT * FROM countries WHERE countries_name IN " + self.list_to_in_condition(bil_country)
			},
			'address_book':{
				'type': 'select',
				'query': "SELECT * FROM address_book WHERE 	customers_id IN " + self.list_to_in_condition(customers_id)
			},


			'zones': {
				'type': 'select',
				'query': "SELECT * FROM  zones INNER JOIN address_book ON zones.zone_id =  address_book.entry_zone_id WHERE address_book.customers_id  IN " + self.list_to_in_condition(customers_id),
			}
		}
	...
	def convert_order_export(self, order, orders_ext):
		...
		customer = get_row_from_list_by_field(orders_ext['data']['orders_customer'], 'customers_id', order['client_customers_id'])
		...
		# customer_name = order['ship_fname'] + ' ' + order['ship_lname']
			order_customer['first_name'] = customer['customers_firstname']
			order_customer['last_name'] = customer['customers_lastname']
		order_data['customer'] = order_customer

		address_book = get_row_from_list_by_field(orders_ext['data']['address_book'], 'customers_id', order['client_customers_id'])
		customer_address = self.construct_order_address()
		#customer_address = self.addConstructDefault(customer_address)
		if customer:
			customer_address['first_name'] = customer['customers_firstname']
			customer_address['last_name'] = customer['customers_lastname']
			customer_address['telephone'] = customer['customers_telephone']
		if address_book :
			customer_address['address_1'] = address_book['entry_street_address']
			# customer_address['address_2'] = order['ship_addr2']
			customer_address['city'] = address_book['entry_city']
			customer_address['postcode'] = address_book['entry_postcode']

			customer_address['company'] = address_book['entry_company']

			customer_country = get_row_from_list_by_field(orders_ext['data']['countries'], 'countries_id', address_book['entry_country_id'])
			if customer_country:
			...
			customer_state = get_row_from_list_by_field(orders_ext['data']['zones'], 'zone_country_id', customer_country['countries_id'])
			if customer_state:
		...
		order_billing['first_name'] = order['billing_name']
		...
		if billing_country:
			...
			billing_state = get_row_from_list_by_field(orders_ext['data']['zones'], 'zone_country_id', billing_country['countries_id'])
			if billing_state:
		...
		# payments = get_row_from_list_by_field(orders_ext['data']['orders_payment'], 'code', order['currency'])
	...
	def get_category_parent(self, category_id):
		query = {
			'type': 'select',
			'query': "SELECT * FROM  categories WHERE id = " + to_str(category_id)
		}
