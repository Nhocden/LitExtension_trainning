...
class LeCartWoocommerce(LeCartWordpress):
	...
	def clear_target_products(self):
		...
		while self._check_product_exists():
			...
			all_meta_query = {
				'type': 'query',
				'query': "DELETE FROM `_DBPRF_postmeta` WHERE post_id IN (SELECT ID FROM _DBPRF_posts)"
			}
			clear_table = self.get_connector_data(self.get_connector_url('query'),
															  {'query': json.dumps(all_meta_query)})
			...
			all_meta_lookup = {
				'type': 'query',
				'query': "DELETE FROM `_DBPRF_wc_product_meta_lookup` WHERE post_id IN (SELECT ID FROM _DBPRF_posts)"
			}
			clear_table = self.get_connector_data(self.get_connector_url('query'),
															  {'query': json.dumps(all_meta_lookup)})
	...
	def category_import(self, convert, category, categories_ext):
		category_term = {
			'name': convert['name'] if convert['name'] else '',
			'slug': convert['name'] if convert['name'] else '',
			'term_group':0
		}
		category_term_query = self.create_insert_query_connector('terms', category_term)
		category_id = self.import_category_data_connector(category_term_query, True, convert['id'])
		self.insert_map(self.TYPE_CATEGORY, convert['id'], category_id)
		if not category_id :
			return response_error('category ' + category_id + ' import false. ')
	...
	def after_category_import(self, category_id, convert, category, categories_ext):
		category_term_taxonomy = {
			...
			'description':convert['description'] if convert['name'] else '',
			'parent': convert['parent']['id'],
			'count' :0
		}
		category_term_taxonomy_query = self.create_insert_query_connector('term_taxonomy', category_term_taxonomy)
		term_id = self.import_data_connector(category_term_taxonomy_query, category_id)
		self.insert_map('term_taxonomy', category_id, term_id)
		if not term_id :
			return response_warning('category_term_taxonomy_query ' + to_str(convert['id']) + ' import false. ')
	...
	def product_import(self, convert, product, products_ext):
		...
		self.insert_map(self.TYPE_PRODUCT, convert['id'], product_id)
	...
	def after_product_import(self, product_id, convert, product, products_ext):
		...
		for key, value in testdb.items():
			product_meta = {
				'post_id': product_id,
				'meta_key': key,
				'meta_value': value if value else ''
			}
			product_query = self.create_insert_query_connector('postmeta', product_meta)
			product_import = self.import_product_data_connector(product_query, True, product_id)
			if not product_import:
				return response_error('Product ' + product_id + ' import false.')
		...
		product_meta_lookup = {
			...
			'virtual': 0,
			'downloadable':0,
			'min_price': convert['price'] if convert['price'] else '',
			'max_price': convert['price'] if convert['price'] else '',
			'onsale':0,
			'stock_quantity':'',
			'stock_status':'instock',
			'rating_count':0,
		...
		product_query1 = self.create_insert_query_connector('wc_product_meta_lookup', product_meta_lookup)
		product_import1 = self.import_product_data_connector(product_query1, True, product_id)
		if not product_import1:
			return response_error('Product ' + product_id + ' import false.')

		# and inport data in term_relationships
		category = convert['categories']
		for value in category :
			term = self.select_map(self._migration_id,self.TYPE_CATEGORY,value['id'] )
			if term :
				term_id = term['id_desc']
				term_taxonomy = self.select_map(self._migration_id, 'term_taxonomy',term_id )
				term_taxonomy_id = term_taxonomy['id_desc'] if term_taxonomy else ''
			...
			term_relationships_data_query = self.create_insert_query_connector('term_relationships', term_relationships_data)
			term_relationships_data_id = self.import_product_data_connector(term_relationships_data_query, True, product_id)
	...
	def prepare_customers_import(self):
		...
		parent = super().prepare_customers_import()
		if self._notice['config']['seo'] or self._notice['config']['seo_301']:
			query = self.dict_to_create_table_sql(self.lecm_rewrite_table_construct())
			self.query_data_connector({'type': 'query', 'query': query['query']})
	...
	def customer_import(self, convert, customer, customers_ext):
		# wp_usermeta, wp_users, wp_wc_customer_lookup
		# self.insert_map(self.TYPE_CUSTOMER, convert['id'], customer_id, convert['code'])
		
		user_data = {
			'user_login': convert['username'] if convert['username'] else convert['last_name'],
			'user_pass': convert['password'] if convert['password'] else '',
			'user_nicename':convert['last_name'],
			'user_email':convert['email'],
			'user_url':'',
			'user_registered':convert['created_at'] if convert['created_at'] else get_current_time(),
			'user_activation_key':'',
			'user_status':0,
			'display_name':convert['username'] if convert['username'] else convert['last_name']
		}
		user_data_query = self.create_insert_query_connector('users', user_data)
		customer_id = self.import_customer_data_connector(user_data_query, True, convert['id'])
		self.insert_map(self.TYPE_CUSTOMER, convert['id'],customer_id)

		if not customer_id :
			return response_error('users ' + customer_id + ' import false. ')
	...
	def after_customer_import(self, customer_id, convert, customer, customers_ext):
		...
		custom_info = {
			'nickname': convert['username'] if convert['username'] else '',
			'first_name': convert['first_name'] if convert['first_name'] else '',
			'last_name': convert['last_name'] if convert['last_name'] else '',

			'billing_first_name': convert['first_name'] if convert['first_name'] else '',
			'billing_last_name':convert['last_name'] if convert['last_name'] else '',
			'billing_company':convert['address'][0]['company'] if convert['address'][0]['company'] else '',
			'billing_address_1':convert['address'][0]['address_1'] if convert['address'][0]['address_1'] else '',
			'billing_address_2':convert['address'][0]['address_2'] if convert['address'][0]['address_2'] else '',
			'billing_city':convert['address'][0]['city'] if convert['address'][0]['city'] else '',
			'billing_postcode':convert['address'][0]['postcode'] if convert['address'][0]['postcode'] else '',
			'billing_country':convert['address'][0]['country']['name'] if convert['address'][0]['country']['name'] else '',
			'billing_state':convert['address'][0]['state']['name'] if convert['address'][0]['state']['name'] else '',
			'billing_phone':convert['address'][0]['telephone'] if convert['address'][0]['telephone'] else '',
			'billing_email':convert['email'] if convert['email'] else '',
			
			'shipping_first_name': convert['first_name'] if convert['first_name'] else '',
			'shipping_last_name':convert['last_name'] if convert['last_name'] else '',
			'shipping_company':convert['address'][0]['company'] if convert['address'][0]['company'] else '',
			'shipping_address_1':convert['address'][0]['address_1'] if convert['address'][0]['address_1'] else '',
			'shipping_address_2':convert['address'][0]['address_2'] if convert['address'][0]['address_2'] else '',
			'shipping_city':convert['address'][0]['city'] if convert['address'][0]['city'] else '',
			'shipping_postcode':convert['address'][0]['postcode'] if convert['address'][0]['postcode'] else '',
			'shipping_country':convert['address'][0]['country']['name'] if convert['address'][0]['country']['name'] else '',
			'shipping_state':convert['address'][0]['state']['name'] if convert['address'][0]['state']['name'] else '',
			'shipping_phone':convert['address'][0]['telephone'] if convert['address'][0]['telephone'] else '',
			'shipping_email':convert['email'] if convert['email'] else ''
		}
		...
		for key, value in custom_info.items():
			...
			user_data_info_query = self.create_insert_query_connector('usermeta', user_data_info)
			customers_info_id = self.import_customer_data_connector(user_data_info_query, True, customer_id)
			if not customers_info_id :
				return response_error('customers info ' + customers_info_id + ' import false. ')

		# wp_usermeta, wp_users, wp_wc_customer_lookup

		customer_lookup = {
			'user_id': customer_id,
			'username': convert['username'] if convert['username'] else convert['first_name'] + ' ' + convert['last_name'],
			'first_name': convert['first_name'],
			'last_name':convert['last_name'],
			'email':convert['email'],
			'date_last_active': convert['updated_at'] if convert['updated_at'] else get_current_time(),
			'date_registered': convert['created_at'],
			'country':convert['address'][0]['country']['name'] if convert['address'][0]['country']['name'] else '',
			'postcode':convert['address'][0]['postcode'] if convert['address'][0]['postcode'] else '',
			'city':convert['address'][0]['city'] if convert['address'][0]['city'] else '',
			'state':convert['address'][0]['state']['name'] if convert['address'][0]['state']['name'] else ''
		}
		customer_lookup_query = self.create_insert_query_connector('wc_customer_lookup', customer_lookup)
		customers_info_id = self.import_customer_data_connector(customer_lookup_query, True, customer_id)
		if not customers_info_id :
			return response_error('customer_lookup ' + customers_info_id + ' import false. ')
	...
	def order_import(self, convert, order, orders_ext):
		self.insert_map('order-customer', convert['id'], order['client_customers_id'])
		items = convert['items']
		for value in items :
			self.insert_map('order-product', convert['id'],value['product']['id'])
			# test = self.select_map(self._migration_id,'order-product', convert['id'])

		customer_id = self.select_map(self._migration_id, self.TYPE_CUSTOMER, order['client_customers_id'] )
		

		date_time = convert['created_at'] if convert['created_at'] else get_current_time()
		date_time = datetime.strptime(date_time, '%Y-%m-%d %H:%M:%S')
		
		post_tittle = date_time.strftime("Order &ndash; %B %d, %Y @  %H:%M %p")

		code_order = date_time.strftime("order %B %d, %Y %H:%M %p")
		# code_order = code_order.replace('.html', '')
		code_order = self.sanitize_title(code_order).strip('-')
		check_slug_exist = True
		while check_slug_exist:
			check_slug_exist = True if self.select_map(self._migration_id, self.TYPE_PRODUCT, None, None, None, code_order) else False
			if check_slug_exist:
				code_order += '-' + to_str(to_int(time.time()))

		# guid= self._notice['target']['cart_url'] + "/?post_type=shop_order&#038;p=" + code_order
		order_post_data = {
			# 'ID': convert['id'],
			'post_author': customer_id if customer_id else convert['customer']['id'],
			'post_date': convert['created_at'] if convert['created_at'] else get_current_time(),
			'post_date_gmt': convert['created_at'] if convert['created_at'] else get_current_time(),
			'post_content': '',
			'post_title': post_tittle,
			'post_excerpt': '',
			'post_status':  'wc-pending',
			'comment_status': 'closed',
			'ping_status': 'closed',
			'post_password': '',
			'post_name': code_order,
			'to_ping': '',
			'pinged': '',
			'post_modified': convert['updated_at'] if convert['updated_at'] else get_current_time(),
			'post_modified_gmt': convert['updated_at'] if convert['updated_at'] else get_current_time(),
			...
			'post_mime_type': '',
			'comment_count': len(convert['items'])
		}

		order_post_query = self.create_insert_query_connector('posts', order_post_data)
		order_id = self.import_order_data_connector(order_post_query, True, convert['id'])
		  
		self.insert_map(self.TYPE_ORDER, convert['id'],order_id)

		if not order_id :
			return response_error('order ' + order_id + ' import false. ')
	...
	def after_order_import(self, order_id, convert, order, orders_ext):
		...
		postmeta_dict = {
			...
			'_billing_first_name': convert['billing_address']['first_name'] if convert['billing_address']['first_name'] else '',
			'_billing_last_name':convert['billing_address']['last_name'] if convert['billing_address']['last_name'] else '',
			'_billing_company':convert['billing_address']['company'] if convert['billing_address']['company'] else '',
			'_billing_address_1':convert['billing_address']['address_1'] if convert['billing_address']['address_1'] else '',
			'_billing_address_2':convert['billing_address']['address_2'] if convert['billing_address']['address_2'] else '',
			'_billing_city':convert['billing_address']['city'] if convert['billing_address']['city'] else '',
			'_billing_postcode':convert['billing_address']['postcode'] if convert['billing_address']['postcode'] else '',
			'_billing_country':convert['billing_address']['country']['name'] if convert['billing_address']['country']['name'] else '',
			'_billing_state':convert['billing_address']['state']['name'] if convert['billing_address']['state']['name'] else '',
			
			'_shipping_first_name': convert['shipping_address']['first_name'] if convert['shipping_address']['first_name'] else '',
			'_shipping_last_name':convert['shipping_address']['last_name'] if convert['shipping_address']['last_name'] else '',
			'_shipping_company':convert['shipping_address']['company'] if convert['shipping_address']['company'] else '',
			'_shipping_address_1':convert['shipping_address']['address_1'] if convert['shipping_address']['address_1'] else '',
			'_shipping_address_2':convert['shipping_address']['address_2'] if convert['shipping_address']['address_2'] else '',
			'_shipping_city':convert['shipping_address']['city'] if convert['shipping_address']['city'] else '',
			'_shipping_postcode':convert['shipping_address']['postcode'] if convert['shipping_address']['postcode'] else '',
			'_shipping_country':convert['shipping_address']['country']['name'] if convert['shipping_address']['country']['name'] else '',
			'_shipping_state':convert['shipping_address']['state']['name'] if convert['shipping_address']['state']['name'] else '',
			}
		for key, value in postmeta_dict.items():
			order_meta = {
				'post_id': order_id,
				'meta_key': key,
				'meta_value': value if value else ''
			}
			order_meta_query = self.create_insert_query_connector('postmeta', order_meta)
			meta_id = self.import_order_data_connector(order_meta_query, True, order_id)
			if not meta_id:
				return response_error('order meta ' + order_id + ' import false.')

		# wp_woocommerce_order_items
		comments_dict = convert['items']
		for value in comments_dict:
			woocommerce_order_items_data = {
				'order_item_name': value['product']['name'],
				'order_item_type': 'line_item',
				'order_id':order_id
			}
			woocommerce_order_items_query = self.create_insert_query_connector('woocommerce_order_items', woocommerce_order_items_data)
			order_item_id = self.import_order_data_connector(woocommerce_order_items_query, True, order_id)
			if not order_item_id:
				return response_error('woocommerce order items ' + order_id + ' import false.')

		
		customer = self.select_map(self._migration_id, self.TYPE_CUSTOMER, order['client_customers_id'] )
		if customer :
			customer_id = customer['id_desc']
		else :
			customer_id = ''
		...
		order_stats_data_query = self.create_insert_query_connector('wc_order_stats', order_stats_data)
		order_stats_data_id = self.import_order_data_connector(order_stats_data_query, True, order_id)
		if not order_stats_data_id:
			return response_error('order stats data ' + order_id + ' import false.')
		...
		for  value in comments_dict:
			comments_data = {
				...
				'comment_author': convert['billing_address']['first_name'] + convert['billing_address']['last_name'],
				'comment_author_email': convert['customer']['email'] if convert['customer']['email'] else '',
				'comment_author_url':'',
				'comment_author_IP':'',
				'comment_date': value['updated_at'] if value['updated_at'] else get_current_time(),
				'comment_date_gmt': value['updated_at'] if value['updated_at'] else get_current_time(),
				'comment_content': 'Added line items: '+ value['product']['name'] ,
				'comment_karma':0,
				'comment_approved':1,
			...
			comments_data_query = self.create_insert_query_connector('comments', comments_data)
			comment_ID = self.import_order_data_connector(comments_data_query, True, order_id)
			if not comment_ID:
				return response_error('order meta ' + order_id + ' import false.')
		...
		# order_product_lookup
			product = self.select_map(self._migration_id, self.TYPE_PRODUCT, value['id'] )
			product_id = product['id_desc'] if product else ''
			order_product_lookup = {
				'order_item_id': order_item_id,
				'order_id': order_id,
				'product_id': product_id,
			...
			order_product_lookup_query = self.create_insert_query_connector('wc_order_product_lookup', order_product_lookup)
			order_product_lookup_id = self.import_order_data_connector(order_product_lookup_query, True, order_id)
			if not order_product_lookup_id:
				return response_error('order product lookup ' + order_id + ' import false.')
		...
		# woocommerce_order_itemmeta
			itemmeta_db = {
				...
				'_qty': value['qty'],
				'_tax_class':'',
				'_line_subtotal':value['subtotal'],
				'_line_subtotal_tax':0,
				'_line_total':value['total'],
				'_line_tax':0,
				'_line_tax_data': php_serialize({
					'total':value['total'],
					'subtotal':0,
			...
			for key, value in itemmeta_db.items():
				woocommerce_order_itemmeta_data = {
					'order_item_id':order_item_id,
					'meta_key': key,
					'meta_value':value
				}	
				woocommerce_order_itemmeta_query = self.create_insert_query_connector('woocommerce_order_itemmeta', woocommerce_order_itemmeta_data)
				woocommerce_order_itemmeta_id = self.import_order_data_connector(woocommerce_order_itemmeta_query, True, order_id)


		# postmeta,comments ,3429,posts,wc_order_product_lookup, wp_wc_order_stats,wp_woocommerce_order_items,woocommerce_order_itemmeta
		return response_success()
